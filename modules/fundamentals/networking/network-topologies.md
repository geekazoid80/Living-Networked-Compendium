...
Think of it as a road hierarchy. Your street (access) connects to the main road through town (distribution), which connects to the motorway (core). Each tier serves a distinct purpose; traffic escalates upward only when it needs to travel further.
...
Where end devices connect: workstations, IP phones, printers, wireless APs. Typically 24- or 48-port switches in wiring closets on each floor.

- Primarily **Layer 2**: Ethernet switching, MAC learning, VLAN assignment
- Port-level policy: PoE, 802.1X authentication, port security, DHCP snooping, storm control
- Two uplinks per switch — one to each distribution switch above it. If one distribution switch fails, the access switch fails over to the other
- No routing here. A device in VLAN 10 wanting to reach VLAN 20 sends traffic up to distribution

One access switch = one physical star. Multiple access switches per distribution pair = a layer of stars aggregated upward.

**Distribution layer — "the main road"**

Aggregates multiple access switches. The **Layer 2 / Layer 3 boundary**.

- **Inter-VLAN routing**: traffic between VLAN 10 (Engineering) and VLAN 20 (Finance) is routed at this layer. Traffic crosses a broadcast domain boundary here.
- **Policy enforcement**: ACLs (block Finance from Engineering), QoS marking (voice traffic tagged for priority handling), route summarisation (one summary route advertised to core, not dozens of specifics)
- **Redundant pair** per building or zone. Each access switch connects to both distribution switches. Each distribution switch connects to both core switches.

**Core layer — "the motorway"**

The high-speed backbone. One job: **move packets fast**. Policy was applied at distribution; core just forwards.

- Pure **Layer 3 routing**, no ACLs, no policy — complex processing would slow it down
- Very high bandwidth: 10G, 40G, 100G links between core and distribution switches
- Always a **redundant pair** — single core switch failure must not take down the network
- Connects to WAN, internet edge, and data centre
...
- **Simplex / half-duplex / full-duplex** — modern switched Ethernet runs full-duplex per port; hubs were half-duplex
- **MTU** = maximum frame payload (1500 bytes standard, 9000 bytes jumbo); **MSS** = MTU minus IP and TCP header overhead (typically 1460 bytes)
- MTU mismatch = fragmentation or black hole; diagnose with sized pings using the DF bit
- **Collision domain** = devices that share a medium and must take turns; a switch gives each port its own collision domain
- **Broadcast domain** = devices that receive all broadcasts; bounded by a router or VLAN — not by a switch alone
- **Unicast** = one recipient; **broadcast** = all in domain; **multicast** = subscribed group only
- **Bus**: shared cable, one collision domain, obsolete for LAN, survives in industrial
- **Ring**: devices in a loop, resilient in dual-ring form, used in carrier networks
- **Star**: all to a central device, dominant in modern Ethernet, central device is a single point of failure
- **Full mesh**: every device to every other, maximum resilience, cable count = N(N−1)/2
- **Partial mesh / hybrid**: backbone of real networks — star at access, partial mesh at distribution/core
- **Three-tier model**: access (L2, end devices) → distribution (L2/L3 boundary, policy, routing) → core (fast L3 forwarding only)
